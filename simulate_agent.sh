...
#	echo -e "\nupdateAgent $2@$1"
	...
	rCode=$( curl -sw %{response_code} -X POST -k https://$server/ns-api/ -dclient_id=jcate@retine -H "Authorization: Bearer $authToken" -dobject=agent -daction=$action -ddomain=$1 -dqueue="4321" -ddevice=sip:$2@$domain -dentry_option=$4 -dentry_action=$5 )
...
function updateSubscriber() {
	rCode=$( curl -sw %{response_code} -X POST -k https://$server/ns-api/ -dclient_id=jcate@retine -H "Authorization: Bearer $authToken" -dobject="subscriber" -daction="update" -ddomain=$1 -duser=$2 --data-urlencode first_name=${first_names[$2]} --data-urlencode last_name=${last_names[$2]} -dmessage=$3 -dnoAuditLog="yes" )
